...
first <- readLines("Boussole/Data/SBEMOOSE/Work/cnv/bous225_03.cnv")
...
#ctd_clean <- ctd[min(which(ctd$pres == max(ctd$pres))):min(which(ctd$pres == min(ctd$pres))),] #for asc
...
ggplot(ctd_clean)+
  ...
  xlim(0,1.5)
echo <- read_table2("Boussole/Data/raw/bouss225_03.txt", 
...
#####attention##################################
###########################################"
write_csv(full_df, 'Boussole/Output/Data/b225_asc3.csv')
