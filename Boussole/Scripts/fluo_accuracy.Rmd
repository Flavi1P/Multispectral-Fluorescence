...
```{r}
library(FactoMineR)
pig_ca <- result_tot_model %>% select(chl_c1_c2:dv_chla) %>% mutate(pig_sum = rowSums(.)) %>% 
  filter(pig_sum > 0) %>% 
  select(-pig_sum)

pig_insitu <- data_ca %>% select(all_of(colnames(pig_ca))) %>%
  mutate(pig_sum = rowSums(.)) %>% 
  filter(pig_sum > 0) %>% 
  select(-pig_sum) %>% 
  mutate(origin = "insitu")

pig_ca <- mutate(pig_ca, origin = "lab")

pig_tot <- bind_rows(pig_ca, pig_insitu)
index <- which(pig_tot$origin == "insitu")

afc <- CA(select(pig_tot, -origin), row.sup = index, graph = FALSE)

pig_and_name <- result_tot_model %>% select(common_name, chl_c1_c2:dv_chla) %>% 
  right_join(.,pig_tot)
...
p <- p + geom_text(data=cPCs, aes(x=PC1,y=PC2,label=rownames(cPCs)),size=10)
...
ggplot(pig_and_name)+
  geom_point(aes(x = PC1, y = PC2, colour = origin))+
  geom_segment(aes(x = 0, y = 0, xend =PC1, yend = PC2), data = vPCs)+
  geom_text_repel(aes(x = PC1, y = PC2, label=rownames(vPCs), fontface = 2), data = vPCs)+
